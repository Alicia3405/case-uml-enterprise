...
export class DetalleSolicitudComponent implements OnInit, OnDestroy {
    ...
    this.workflowService.obtenerPorId(id).subscribe({
      next: (res) => {
        if (res.datos) {
          ...
          // Sólo actualiza la señal si hubo cambios de estado o historial para evitar reflows pesados innecesarios
          if (current && (current.estado !== incoming.estado || current.historial?.length !== incoming.historial?.length || current.departamentoActual !== incoming.departamentoActual || current.usuarioAsignado !== incoming.usuarioAsignado)) {
  ...
  private cargarConfiguracionFormulario(workflowKey: string, tareaId: string) {
    this.workflowSupportService.obtenerWorkflowDefinition(workflowKey).subscribe({
      next: (def: any) => {
        if (def && def.xml) {
          this.extraerCamposDeXml(def.xml, tareaId);
          this.extraerTodasLasTareasDeXml(def.xml);
        }
      }
    });
  }

  private extraerTodasLasTareasDeXml(xml: string) {
    if (!xml) return;
    try {
  ...
  private extraerCamposDeXml(xml: string, tareaId: string) {
    if (!xml || !tareaId) {
      this.formFields.set([]);
      return;
    }
...
`;
  ...
  descargarArchivo(nombreAlmacenado: string | undefined | null) {
    ...
    window.open(`${this.basePath}/api/v1/archivos/${nombreAlmacenado}`, '_blank');
  }
