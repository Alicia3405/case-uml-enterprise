...
export class DocumentEditorComponent implements OnInit, OnDestroy {
  ...
  restaurarVersion(version: VersionDocumento) {
    if (!this.estaBloqueadoPorMi()) return;
    if (confirm(`¿Estás seguro de que deseas reemplazar el borrador actual con el contenido de la Versión ${version.version}?`)) {
      this.contenido = version.contenidoColaborativoSnapshot || '';
      this.onContentChange();
...
- Directrices de Formato e Instrucciones para Gemini AI:
...
`;
  ...
  askAi(prompt: string) {
    ...
    this.aiMessages.update(msgs => [...msgs, { sender: 'assistant', text: 'Analizando con Gemini AI...', timestamp: new Date(), loading: true }]);
