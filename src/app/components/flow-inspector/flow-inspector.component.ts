...
export class FlowInspectorComponent {
  ...
  totalGeneral = computed(() => {
    const c = this.contadores() as Record<string, number>;
    return (c['PENDIENTE'] || 0) + (c['EN_REVISION'] || 0) + (c['APROBADO'] || 0) + (c['RECHAZADO'] || 0);
  });

  readonly flowSteps = [
    { estado: 'PENDIENTE' as EstadoWorkflow, label: 'PENDIENTE', color: ESTADO_VISUAL_CONFIG['PENDIENTE']?.color || '#64748b' },
    { estado: 'EN_REVISION' as EstadoWorkflow, label: 'REVISION', color: ESTADO_VISUAL_CONFIG['EN_REVISION']?.color || '#2563eb' },
    { estado: 'APROBADO' as EstadoWorkflow, label: 'APROBADO', color: ESTADO_VISUAL_CONFIG['APROBADO']?.color || '#16a34a' },
    { estado: 'RECHAZADO' as EstadoWorkflow, label: 'RECHAZADO', color: ESTADO_VISUAL_CONFIG['RECHAZADO']?.color || '#dc2626' },
  ];
